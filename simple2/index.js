	...
	initGL: function (camera, object, renderer) {
		...
		baseGeometry.setIndex(new THREE.BufferAttribute(idx, 1));
			...
			vertexShader: `
                                ...
                                // --- TWEAKABLE SETTINGS ---
                                // How opaque a splat must be to render (0.0 to 1.0). Higher = less background fuzz, more performance.
                                #define ALPHA_CUTOFF 0.15 
                                // How large to draw the quads. 0.3535 matches the old 0.5 radius discard. Bump to 0.5 or 1.0 if you see holes.
                                ...
                                void main() {
                                        ...
                                        // 2. Alpha Culling (Early Out)
                                        float baseAlpha = float(colorUint >> 24) * 0.003921569;
                                        if (baseAlpha * fadeOpacity < ALPHA_CUTOFF) {
                                                gl_Position = vec4(0.0, 0.0, 2.0, 1.0); // Collapse to nothing
                                                return;                                 // Skip all the heavy matrix math!
                                        }
                                        ...
                                        vColor = vec4(
                                                vec3(colorUint & 0xFFu, (colorUint >> 8) & 0xFFu, (colorUint >> 16) & 0xFFu) * 0.003921569,
                                                1.0 // Force alpha to 1.0. 
                                        );
	...
	getModelViewMatrix: function (camera) {
        ...
        createWorker: function (self) {
				...
				const radius = matrices[offset + 15] * scaleFactor;
                                ...
                                const skipCullEdges = (radiusTransparencyProduct / scaleFactor) > 0.1;
				const skipCullBehind = (radiusTransparencyProduct / scaleFactor) > 0.35;
