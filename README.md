...
When browsers support the `multiview` extension, the code creates a projection layer with `layout: stereo-multiview`. This single-pass stereo technique renders both eyes simultaneously for improved performance. The feature is requested as optional so the scene still functions on devices without multiview support.
