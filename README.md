...
The `space-warp` option attempts to enable WebXR Space Warp on compatible browsers. When available, Space Warp synthesizes intermediate frames so the application can run at a lower frame rate while maintaining smooth visuals. The session's render state is updated when VR mode starts, enabling Space Warp and, when supported, targeting a 45&nbsp;FPS frame rate.
