...
Hold a single controller grip, trigger, or hand pinch to move the loaded splat. When both hands are gripping, pulling the trigger, or pinching the object, you can scale it and rotate it around whichever of the vertical (Y) or pitch (X) axes most closely matches the twist of your hands. The midpoint between the hands is used as the pivot for these transformations.
