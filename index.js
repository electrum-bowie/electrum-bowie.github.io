	...
	getModelViewMatrix: function (camera) {
        ...
        createWorker: function (self) {
                ...
                const filterSplats = function filterSplats(matrices, view, mvp, scaleFactor = 1.0, focal = 1.0) {
                        ...
                        cache.maxDepth = -Infinity;
                        cache.minDepth = Infinity;
                        ...
                        for (let i = 0; i < vertexCount; i++) {
                                ...
                                depthList[visibleCount] = depth;
                                validIndexList[visibleCount] = i;
                                visibleCount++;
                                if (depth > cache.maxDepth) cache.maxDepth = depth;
                                if (depth < cache.minDepth) cache.minDepth = depth;
                        }
                };

                const sortSplats = function sortSplats() {
                        let validCount = visibleCount;
                        ...
                        let maxDepth = cache.maxDepth;
                        let minDepth = cache.minDepth;
                        ...
                        for (let i = 0, j = validCount - 1; i < validCount; i++, j--) {
                                result[i] = depthIndex[j];
                        }
		...
		self.onmessage = (e) => {
                        ...
                        if (e.data.method == "filter") {
                                if (matrices === undefined) {
                                        ...
                                        fadeCopy[0] = 2.0;
                                        self.postMessage({ type: "filter", fadeOpacities: fadeCopy }, [fadeCopy.buffer]);
                                } else {
                        ...
                        if (e.data.method == "sort") {
                                if (matrices === undefined) {
                                        ...
                                        self.postMessage({ type: "sort", sortedIndexes }, [sortedIndexes.buffer]);
                                } else {
