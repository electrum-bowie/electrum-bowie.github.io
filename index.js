AFRAME.registerComponent("gaussian_splatting", {
	...
	initGL: function (camera, object, renderer) {
		...
		this.renderer.outputColorSpace = THREE.SRGBColorSpace;
                ...
                this.basicVisible = new Uint32Array(0);
                this.occlusionVisible = new Uint32Array(0);
                this.occlusionVisibleSet = new Set();
                this.basicWorkerReady = false;
		...
		material.onBeforeRender = ((renderer, scene, camera, geometry, object, group) => {
			...
			mesh.material.uniforms.gsProjectionMatrix.value = projectionMatrix;
                ...
                this.basicWorker = new Worker(
                        URL.createObjectURL(
                                new Blob(["(", this.createBasicWorker.toString(), ")(self)"], {
                ...
                this.basicWorker.onmessage = (e) => {
                        this.basicVisible = new Uint32Array(e.data.candidates);
                        this.basicWorkerReady = true;
                        this.updateFinalVisibility();
                };
                this.occlusionWorker.onmessage = (e) => {
                        this.occlusionVisible = new Uint32Array(e.data.sorted);
                        this.occlusionVisibleSet = new Set(Array.from(this.occlusionVisible));
        ...
        tick: function (time, timeDelta) {
                ...
                if (this.basicWorkerReady && this.occlusionWorkerReady && (camPosChanged || camRotChanged || objPosChanged || objRotChanged || scaleChanged)) {
                        this.sortSplatsNow();
                }
        ...
        sortSplatsNow: function () {
                if (!this.basicWorkerReady || !this.occlusionWorkerReady) return;
                this.basicWorkerReady = false;
                this.occlusionWorkerReady = false;
                ...
                let view2 = new Float32Array([camera_mtx[2], camera_mtx[6], camera_mtx[10], camera_mtx[14]]);
                let mvp2 = new Float32Array(mvpMatrix.elements);
                const indicesForWorker = new Uint32Array(this.basicVisible);
                this.occlusionWorker.postMessage({
                        ...
                        sliderValue: sliderValue,
                        focal: focal,
                        indexes: indicesForWorker.buffer,
                }, [view2.buffer, mvp2.buffer, indicesForWorker.buffer]);
	...
	getModelViewMatrix: function (camera) {
        ...
        updateFinalVisibility: function () {
                if (!this.mesh || !this.mesh.geometry) return;
                let final = [];
                for (let i = 0; i < this.basicVisible.length; i++) {
                        const idx = this.basicVisible[i];
                        if (this.occlusionVisibleSet.has(idx)) final.push(idx);
                }
                let indexes = new Uint32Array(final);
                ...
                if (this.basicWorkerReady && this.occlusionWorkerReady) this.sortReady = true;
        },
        ...
        createBasicWorker: function (self) {
                let matrices = undefined;
                ...
                const basicCull = async function basicCull(matrices, view, mvp, scaleFactor = 1.0, sliderValue = 1, focal = 1.0) {
                        ...
                        const vertexCount = matrices.length / 16;
                        let threshold = -0.001;
                        let result = new Uint32Array(vertexCount);
                        let count = 0;
                        for (let i = 0; i < vertexCount; i++) {
                                const px = matrices[i * 16 + 12];
                                const py = matrices[i * 16 + 13];
                ...
                self.onmessage = async (e) => {
                        ...
                        if (e.data.method == "basicCull") {
                                if (matrices === undefined) {
                                        const candidates = new Uint32Array(0);
                                        ...
                                        const sliderValue = typeof e.data.sliderValue === 'number' ? e.data.sliderValue : 1;
                                        const focal = typeof e.data.focal === 'number' ? e.data.focal : 1.0;
                                        const candidates = await basicCull(matrices, view, mvp, scaleFactor, sliderValue, focal);
                                        self.postMessage({ candidates }, [candidates.buffer]);
                                }
                        }
                };
        ...
        createOcclusionWorker: function (self) {
                ...
                const occlusionSort = async function occlusionSort(matrices, indexes, view, mvp, scaleFactor = 1.0, sliderValue = 1, focal = 1.0) {
                        const vertexCount = indexes.length;
                ...
                self.onmessage = async (e) => {
                        ...
                        if (e.data.method == "occlusionSort") {
                                if (matrices === undefined) {
                                        const sorted = new Uint32Array(0);
                                        ...
                                        const sliderValue = typeof e.data.sliderValue === 'number' ? e.data.sliderValue : 1;
                                        const focal = typeof e.data.focal === 'number' ? e.data.focal : 1.0;
                                        const sorted = await occlusionSort(matrices, indexes, view, mvp, scaleFactor, sliderValue, focal);
                                        self.postMessage({ sorted }, [sorted.buffer]);
                                }
                        }
                };
