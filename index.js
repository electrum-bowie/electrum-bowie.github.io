	...
	initGL: function (camera, object, renderer) {
			...
			URL.createObjectURL(
                ...
                this.worker.onmessage = (e) => {
                        ...
                        if (e.data.fadeOpacities) {
                                ...
                                for (let offset = 0, y = 0; offset < fades.length; y++, offset += 4096) {
                                        const row = fades.subarray(offset, offset + 4096);
                                        gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, y, row.length, 1, gl.RED, gl.UNSIGNED_BYTE, row);
                                }
		...
		while (vertexCount > 0) {
			...
			gl.texSubImage2D(gl.TEXTURE_2D, 0, xoffset, yoffset, width, height, gl.RGBA, gl.FLOAT, this.centerAndScaleData, this.loadedVertexCount * 4);
                        ...
                        const fadeOpacityTextureProperties = this.renderer.properties.get(this.fadeOpacityTexture);
                        gl.bindTexture(gl.TEXTURE_2D, fadeOpacityTextureProperties.__webglTexture);
                        const fadeChunk = new Uint8Array(width * height);
                        fadeChunk.fill(255);
                        gl.texSubImage2D(gl.TEXTURE_2D, 0, xoffset, yoffset, width, height, gl.RED, gl.UNSIGNED_BYTE, fadeChunk);
