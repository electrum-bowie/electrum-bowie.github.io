AFRAME.registerComponent("gaussian_splatting", {
	...
	initGL: function (camera, object, renderer) {
		...
		material.onBeforeRender = ((renderer, scene, camera, geometry, object, group) => {
			...
			mesh.material.uniforms.gsProjectionMatrix.value = projectionMatrix;
                ...
                this.basicWorker.onmessage = (e) => {
                        this.lastBasicIndexes = new Uint32Array(e.data.indexes);
                        this.basicReady = true;
                        if (this.occlusionReady) this.occlusionSort();
                };
        ...
        tick: function (time, timeDelta) {
                ...
                if (this.basicReady && this.occlusionReady && (camPosChanged || camRotChanged || objPosChanged || objRotChanged || scaleChanged)) {
                        this.sortSplatsNow();
                }
        ...
        sortSplatsNow: function () {
                ...
                this.basicWorker.postMessage({
                        ...
                        focal: focal,
                }, [view.buffer.slice(0), mvp.buffer.slice(0)]);

                let indexesBuffer = this.lastBasicIndexes ? this.lastBasicIndexes.buffer.slice(0) : undefined;
                let transfer = [view.buffer, mvp.buffer];
                if (indexesBuffer) transfer.push(indexesBuffer);
                this.occlusionWorker.postMessage({
                        method: "sort",
                        view: view.buffer,
                        mvp: mvp.buffer,
	...
	getModelViewMatrix: function (camera) {
        ...
        createBasicWorker: function (self) {
                ...
                self.onmessage = (e) => {
                        ...
                        if (e.data.method == "cull") {
                                if (matrices === undefined) {
                                        const indexes = new Uint32Array(0);
                                        self.postMessage({ indexes }, [indexes.buffer]);
                                } else {
                                        const view = new Float32Array(e.data.view);
                                        const mvp = new Float32Array(e.data.mvp);
                                        const scaleFactor = typeof e.data.scale === 'number' ? e.data.scale : 1.0;
                                        const sliderValue = typeof e.data.sliderValue === 'number' ? e.data.sliderValue : 1;
                                        const focal = typeof e.data.focal === 'number' ? e.data.focal : 1.0;
                                        const indexes = basicCull(matrices, view, mvp, scaleFactor, sliderValue, focal);
                                        self.postMessage({ indexes }, [indexes.buffer]);
                                }
                        }
                };
        ...
        createOcclusionWorker: function (self) {
                ...
                const occlusionSort = function(matrices, indices, view, mvp, scaleFactor = 1.0, sliderValue = 1, focal = 1.0) {
                        const sizeThreshold = 0.00001 * (isNaN(sliderValue) ? 1 : sliderValue);
                        const vertexCount = indices ? indices.length : matrices.length / 16;
                        let threshold = -0.001;
                ...
                self.onmessage = (e) => {
                        ...
                        if (e.data.method == "sort") {
                                if (matrices === undefined) {
                                        ...
                                        const sliderValue = typeof e.data.sliderValue === 'number' ? e.data.sliderValue : 1;
                                        const focal = typeof e.data.focal === 'number' ? e.data.focal : 1.0;
                                        const indexes = e.data.indexes ? new Uint32Array(e.data.indexes) : undefined;
                                        const sortedIndexes = occlusionSort(matrices, indexes, view, mvp, scaleFactor, sliderValue, focal);
                                        self.postMessage({ sortedIndexes }, [sortedIndexes.buffer]);
                                }
                        }
                };
