	...
	getModelViewMatrix: function (camera) {
        ...
        createWorker: function (self) {
			...
			for (let i = 1; i < 256 * 256; i++) starts0[i] = starts0[i - 1] + counts0[i - 1];
				...
				const minX = Math.max(0, Math.min(gridSizeX - 2, i0));
				const maxX = Math.max(0, Math.min(gridSizeX - 2, i1));
				const minY = Math.max(0, Math.min(gridSizeY - 2, j0));
				const maxY = Math.max(0, Math.min(gridSizeY - 2, j1));
